...
export class ThemeService {
  ...
  private readonly currentModeSignal = signal<ThemeMode>('system');

  readonly currentMode = this.currentModeSignal.asReadonly();
  ...
  private applyMode(mode: ThemeMode): void {
    ...
    const shouldUseDarkMode = mode === 'dark' || (mode === 'system' && prefersDark);

    this.document.documentElement.classList.toggle('driveos-dark', shouldUseDarkMode);
  }
  ...
  private readonly systemThemeListener = (): void => {
    if (this.currentModeSignal() === 'system') {
