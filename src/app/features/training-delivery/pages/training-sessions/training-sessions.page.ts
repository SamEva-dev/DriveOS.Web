...
export class TrainingSessionsPage {
  ...
  readonly activeFilterCount = computed(
    () =>
      [
        this.status(),
        this.attendance(),
        this.attention() !== 'all' ? this.attention() : null,
  ...
  readonly filteredSessions = computed(() => {
    ...
    return this.sessions().filter((session) => {
      ...
      if (this.attendance() !== null && session.attendanceStatus !== this.attendance())
        return false;
      if (
        q &&
        !`${session.studentDisplayName} ${session.trainingCategory ?? ''} ${session.meetingPoint ?? ''}`
          .toLocaleLowerCase()
          .includes(q)
      )
        return false;
      switch (this.attention()) {
        case 'reports':
          return this.isMissingReport(session);
        case 'incidents':
          return session.hasOpenIncident;
        case 'duration':
          return this.hasDurationVariance(session);
        case 'late':
          return this.isLate(session);
        default:
          return true;
      }
    });
  ...
  readonly totalCompleted = computed(
    () => this.filteredSessions().filter((x) => x.status === 4).length,
  );
  readonly totalInProgress = computed(
    () => this.filteredSessions().filter((x) => x.status === 3 || x.status === 6).length,
  );
  readonly totalAttention = computed(
    () =>
      this.filteredSessions().filter(
        (x) =>
          this.isMissingReport(x) ||
          x.hasOpenIncident ||
          this.hasDurationVariance(x) ||
          this.isLate(x),
      ).length,
  );
  ...
  setAttendance(value: string): void {
    this.attendance.set(value === '' ? null : Number(value));
  }
  setAttention(value: string): void {
    this.attention.set(value as AttentionFilter);
  }
  resetFilters(): void {
    this.query.set('');
    this.status.set(null);
    this.attendance.set(null);
    this.attention.set('all');
  }
  ...
  plannedDurationMinutes(s: TrainingDeliveryDashboardSession): number {
    return Math.max(
      0,
      Math.round(
  ...
  durationDifferenceMinutes(s: TrainingDeliveryDashboardSession): number | null {
    return s.deliveredDurationMinutes === null
      ? null
      : s.deliveredDurationMinutes - this.plannedDurationMinutes(s);
  }
  isMissingReport(s: TrainingDeliveryDashboardSession): boolean {
    return s.status === 4 && !s.hasReport;
  }
  isLate(s: TrainingDeliveryDashboardSession): boolean {
    return (
      new Date(s.plannedStartAtUtc).getTime() < Date.now() && (s.status === 1 || s.status === 2)
    );
  }
  hasDurationVariance(s: TrainingDeliveryDashboardSession): boolean {
    const d = this.durationDifferenceMinutes(s);
    return d !== null && Math.abs(d) >= 15;
  }
  hasAttention(s: TrainingDeliveryDashboardSession): boolean {
    return (
      this.isMissingReport(s) || s.hasOpenIncident || this.hasDurationVariance(s) || this.isLate(s)
    );
  }
