...
export class TrainingSessionDetailPage implements OnDestroy {
  ...
  readonly plannedDurationMinutes = computed(() => {
    ...
    return Math.max(
      0,
      Math.round(
        (new Date(s.plannedEndAtUtc).getTime() - new Date(s.plannedStartAtUtc).getTime()) / 60000,
      ),
    );
  });
  readonly activeInterruption = computed(
    () => this.session()?.interruptions.some((x) => x.isActive) ?? false,
  );
  readonly inProgress = computed(() => this.session()?.status === 3);
  readonly isInterrupted = computed(
    () => this.session()?.status === 6 || this.activeInterruption(),
  );
  ...
  readonly elapsedMinutes = computed(() => {
    ...
    const end = this.session()?.actualEndAtUtc
      ? new Date(this.session()!.actualEndAtUtc!).getTime()
      : this.now();
    return Math.max(0, Math.floor((end - new Date(startedAt).getTime()) / 60000));
  });
  readonly canRecordObservation = computed(() =>
    this.authorization.hasPermission(TRAINING_DELIVERY_PERMISSIONS.execution.observationRecord),
  );
  readonly canRecordIntervention = computed(() =>
    this.authorization.hasPermission(TRAINING_DELIVERY_PERMISSIONS.execution.interventionRecord),
  );
  readonly canInterrupt = computed(() =>
    this.authorization.hasPermission(TRAINING_DELIVERY_PERMISSIONS.execution.interrupt),
  );
  readonly canResume = computed(() =>
    this.authorization.hasPermission(TRAINING_DELIVERY_PERMISSIONS.execution.resume),
  );
  readonly canReportIncident = computed(() =>
    this.authorization.hasPermission(TRAINING_DELIVERY_PERMISSIONS.incidents.report),
  );
  readonly canComplete = computed(() =>
    this.authorization.hasPermission(TRAINING_DELIVERY_PERMISSIONS.sessions.complete),
  );
  readonly canRecordVehicleUsage = computed(() =>
    this.authorization.hasPermission('TrainingDelivery.Execution.Odometer.Record'),
  );
  readonly canCorrectAttendance = computed(() =>
    this.authorization.hasPermission(TRAINING_DELIVERY_PERMISSIONS.attendance.correct),
  );
  readonly canOverrideAttendance = computed(() =>
    this.authorization.hasPermission(TRAINING_DELIVERY_PERMISSIONS.attendance.override),
  );
  readonly attendanceDurationMinutes = computed(() => {
    ...
    return Math.max(
      0,
      Math.round(
        (new Date(attendance.actualDepartureAtUtc).getTime() -
          new Date(attendance.actualArrivalAtUtc).getTime()) /
          60000,
      ),
    );
  });
  readonly plannedDurationDeltaMinutes = computed(() => {
    ...
    return delivered === null || delivered === undefined
      ? null
      : delivered - this.plannedDurationMinutes();
  });
  readonly openIncidents = computed(() =>
    this.incidents().filter((x) => x.status !== 5 && x.status !== 6),
  );
  readonly canPrepare = computed(() =>
    this.authorization.hasPermission(TRAINING_DELIVERY_PERMISSIONS.sessions.prepare),
  );
  readonly canStart = computed(() =>
    this.authorization.hasPermission(TRAINING_DELIVERY_PERMISSIONS.sessions.start),
  );
  ...
  readonly readinessPassedCount = computed(
    () => this.preparation()?.checks.filter((x) => x.status === 1).length ?? 0,
  );
  readonly readinessBlockedCount = computed(
    () => this.preparation()?.checks.filter((x) => x.status === 2).length ?? 0,
  );
  readonly readinessReviewCount = computed(
    () => this.preparation()?.checks.filter((x) => x.status === 3).length ?? 0,
  );
  ...
  readonly studentEnrollmentActive = computed(
    () => this.session() !== null && this.studentStatuses()?.enrollmentStatus === 'Active',
  );
  readonly studentSchedulingAllowed = computed(() => {
    const statuses = this.studentStatuses();
    if (!statuses) return false;
    return (
      statuses.schedulingStatus !== 'Blocked' &&
  ...
  readonly studentHasBlockingRestriction = computed(() => {
    const statuses = this.studentStatuses();
    if (!statuses) return false;
    return statuses.blocks.some(
      (block) =>
        block.status === 'Active' &&
        (this.hasBlockingAction(block.blockingActions, 'Schedule') ||
          this.hasBlockingAction(block.blockingActions, 'StartLesson')),
    );
  });
  readonly vehicleAssigned = computed(() => !!this.session()?.vehicleId);
  readonly vehicleComplianceCheck = computed(
    () =>
      this.preparation()?.checks.find(
        (x) => x.code.toLowerCase().includes('fleet') || x.code.toLowerCase().includes('vehicle'),
      ) ?? null,
  );
  ...
  readonly historyCount = computed(() => {
    ...
    if (!s) return 0;
    return (
      s.attendanceHistory.length +
      s.interventions.length +
      s.observations.length +
      s.markers.length +
      s.interruptions.length +
      s.odometerReadings.length +
      s.energyEntries.length +
      s.competencyAssessments.length
    );
  });
  ...
  readonly tabs: readonly { key: SessionDetailTab; labelKey: string; icon: string }[] = [
    ...
    {
      key: 'participants',
      labelKey: 'training.sessionDetail.tabs.participants',
      icon: 'ph ph-users-three',
    },
    {
      key: 'preparation',
      labelKey: 'training.sessionDetail.tabs.preparation',
      icon: 'ph ph-check-square-offset',
    },
    {
      key: 'attendance',
      labelKey: 'training.sessionDetail.tabs.attendance',
      icon: 'ph ph-user-check',
    },
    {
      key: 'execution',
      labelKey: 'training.sessionDetail.tabs.execution',
      icon: 'ph ph-steering-wheel',
    },
    { key: 'report', labelKey: 'training.sessionDetail.tabs.report', icon: 'ph ph-clipboard-text' },
    {
      key: 'incidents',
      labelKey: 'training.sessionDetail.tabs.incidents',
      icon: 'ph ph-warning-octagon',
    },
    {
      key: 'history',
      labelKey: 'training.sessionDetail.tabs.history',
  ...
  openTechnical(): void {
    this.drawer.set('technical');
  }
  openPreparation(): void {
    this.drawer.set('preparation');
  }
  openPrestartCheck(): void {
    this.drawer.set('prestartCheck');
  }
  openStart(): void {
  ...
  openIncident(incident: TrainingIncidentDetail): void {
    this.selectedIncident.set(incident);
    this.drawer.set('incident');
  }
  openQuickObservation(): void {
    this.resetExecutionFeedback();
    this.drawer.set('quickObservation');
  }
  openVehicleUsage(): void {
    this.vehicleUsageErrors.set([]);
    this.vehicleMileage.set(this.session()?.latestOdometerKilometers ?? null);
    this.vehicleEnergyLevel.set(
      this.session()?.latestEnergyLevelPercent ?? this.session()?.endEnergyLevelPercent ?? null,
    );
  ...
  openQuickIntervention(): void {
    this.resetExecutionFeedback();
    this.drawer.set('quickIntervention');
  }
  openInterrupt(): void {
    this.resetExecutionFeedback();
    this.drawer.set('interrupt');
  }
  openReportIncident(): void {
    this.resetExecutionFeedback();
    this.drawer.set('reportIncident');
  }
  openFinish(): void {
  ...
  recordQuickObservation(): void {
    ...
    this.executeSessionMutation(
      this.api.recordObservation(this.sessionId, {
  ...
  recordMarker(): void {
    ...
    this.executeSessionMutation(
      this.api.recordMarker(this.sessionId, {
        operationId: crypto.randomUUID(),
  ...
  recordQuickIntervention(): void {
    ...
    this.api
      .recordIntervention(this.sessionId, {
        operationId,
      ...
      .subscribe({
        next: (session) => {
          ...
          const createIncident =
            this.interventionCreateIncident() &&
            this.canReportIncident() &&
            this.interventionSeverity() >= 3;
          if (!createIncident) {
            this.finishInterventionSave();
            return;
          }
          this.api
            .reportIncident(this.sessionId, {
              operationId: crypto.randomUUID(),
              incidentType: 1,
              severity: this.interventionSeverity(),
              occurredAtUtc: new Date().toISOString(),
              description: `${context} — ${reason}`,
              immediateActions: this.interventionOutcome().trim() || reason,
  ...
  private finishInterventionSave(): void {
    this.executionSaving.set(false);
    this.drawer.set(null);
    this.interventionContext.set('');
    this.interventionReason.set('');
    this.interventionCompetencyId.set(null);
    this.interventionOutcome.set('');
    this.interventionInternalComment.set('');
    this.interventionSharedExplanation.set('');
    this.interventionCreateIncident.set(false);
  }

  interruptSession(): void {
    if (!this.inProgress() || !this.canInterrupt() || this.activeInterruption()) return;
    this.executeSessionMutation(
      this.api.interruptSession(this.sessionId, {
        operationId: crypto.randomUUID(),
  ...
  resumeSession(): void {
    ...
    this.executeSessionMutation(
      this.api.resumeSession(this.sessionId, {
        operationId: crypto.randomUUID(),
  ...
  reportQuickIncident(): void {
    ...
    if (!this.inProgress() || !this.canReportIncident() || !description || !immediateActions)
      return;
    this.executionSaving.set(true);
    this.executionErrors.set([]);
    this.api
  ...
  saveVehicleUsage(): void {
    ...
    if (mileage !== null && mileage !== session.latestOdometerKilometers) {
      if (session.latestOdometerKilometers !== null && mileage < session.latestOdometerKilometers) {
        this.vehicleUsageErrors.set(['errors.trainingDelivery.session.odometer.mustBeMonotonic']);
        return;
      }
      calls.push(
        this.api.recordOdometer(this.sessionId, {
          operationId: crypto.randomUUID(),
          odometerKilometers: mileage,
          source: 2,
          observedAtUtc: now,
        }),
      );
    ...
    if (level !== null || (this.vehicleEnergyType() !== 1 && quantity !== null)) {
      calls.push(
        this.api.recordEnergy(this.sessionId, {
          operationId: crypto.randomUUID(),
          type: this.vehicleEnergyType(),
          energyLevelPercent: level,
          quantity: this.vehicleEnergyType() === 1 ? null : quantity,
          observedAtUtc: now,
          note: this.vehicleEnergyNote().trim() || null,
    ...
    if (this.vehicleAnomaly() && this.vehicleAnomalyDescription().trim()) {
      calls.push(
        this.api.reportIncident(this.sessionId, {
          ...
          severity: 2,
          occurredAtUtc: now,
          description: this.vehicleAnomalyDescription().trim(),
          immediateActions:
            this.vehicleEnergyNote().trim() || 'Vehicle anomaly recorded from SCR-SES-013',
          additionalParticipants: [],
        }),
      );
    }
    ...
    forkJoin(calls).subscribe({
      next: () => {
        this.vehicleUsageSaving.set(false);
        this.closeDrawer();
        this.load();
      },
      error: (error: HttpErrorResponse) => {
        this.vehicleUsageSaving.set(false);
        this.vehicleUsageErrors.set(this.apiErrors.getMessages(error));
      },
    });
  }
  ...
  saveAttendanceCorrection(): void {
    ...
    if (useOverride && (!this.canOverrideAttendance() || !this.attendanceOverrideReason().trim()))
      return;
  ...
  finishSession(): void {
    ...
    if (
      this.finishVehicleIssue() &&
      (!this.finishVehicleIssueDescription().trim() || !this.finishVehicleIssueActions().trim())
    ) {
    ...
    const operations: Array<
      | ReturnType<TrainingDeliveryApiService['recordOdometer']>
      | ReturnType<TrainingDeliveryApiService['reportIncident']>
    > = [];
    if (mileage !== null && (currentMileage === null || mileage !== currentMileage)) {
      operations.push(
        this.api.recordOdometer(this.sessionId, {
    ...
    if (this.finishVehicleIssue()) {
      operations.push(
        this.api.reportIncident(this.sessionId, {
          ...
          severity: 2,
          occurredAtUtc: endAt,
          description: this.finishVehicleIssueDescription().trim(),
          immediateActions: this.finishVehicleIssueActions().trim(),
          additionalParticipants: [],
        }),
      );
    }
    ...
    const finish = () =>
      this.api
        .finishSession(this.sessionId, {
          operationId: crypto.randomUUID(),
          actualEndAtUtc: endAt,
          endEnergyLevelPercent: this.finishEnergyLevel(),
        })
        .subscribe({
          next: (updated) => {
            this.session.set(updated);
            ...
            if (this.finishMode() === 'now')
              void this.router.navigate(['/training/sessions', this.sessionId, 'report']);
          },
          error: (error: HttpErrorResponse) => {
            this.finishErrors.set(this.apiErrors.getMessages(error));
            this.finishSaving.set(false);
          },
        });
  ...
  private executeSessionMutation(
    request: import('rxjs').Observable<TrainingSessionDetail>,
    onSuccess?: () => void,
  ): void {
    this.executionSaving.set(true);
    this.executionErrors.set([]);
  ...
  statusKey(value: number): string {
    return `training.statuses.session.${value}`;
  }
  attendanceKey(value: number | null): string {
    return value === null
      ? 'training.statuses.attendance.none'
      : `training.statuses.attendance.${value}`;
  }
  incidentSeverityKey(value: number): string {
    return `training.sessionDetail.incidentSeverity.${value}`;
  }
  incidentStatusKey(value: number): string {
    return `training.sessionDetail.incidentStatus.${value}`;
  }
  observationTypeKey(value: number): string {
    return `training.sessionDetail.observationType.${value}`;
  }
  markerTypeKey(value: number): string {
    return `training.sessionDetail.marker.type.${value}`;
  }
  markerSeverityKey(value: number): string {
    return `training.sessionDetail.marker.severity.${value}`;
  }
  interventionTypeKey(value: number): string {
    return `training.sessionDetail.intervention.type.${value}`;
  }
  interventionSeverityKey(value: number): string {
    return `training.sessionDetail.interventionSeverity.${value}`;
  }
  readinessStatusKey(value: number): string {
    return `training.sessionDetail.preparation.status.${value}`;
  }
