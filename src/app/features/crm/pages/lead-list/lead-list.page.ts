...
import { DriveOsBadgeComponent, DriveOsBadgeVariant, DriveOsButtonComponent,
  DriveOsEmptyStateComponent, DriveOsInputDirective, DriveOsPageChange,
  DriveOsPaginatorComponent, DriveOsSpinnerComponent, DriveOsStateBannerComponent,
  DriveOsTableDirective } from '../../../../shared/ui';
...
type LeadListState = 'nominal' | 'empty' | 'noResults' | 'loading' | 'error' | 'partialData'
  | 'readOnly' | 'limitedPermission' | 'duplicates' | 'staleData';

interface StateTab { readonly id: LeadListState; readonly permission: string; }

@Component({
  selector: 'driveos-lead-list-page', standalone: true, providers: [LeadsListStore],
  ...
  templateUrl: './lead-list.page.html', changeDetection: ChangeDetectionStrategy.OnPush,
})
export class LeadListPage {
  ...
  readonly stateTabsVisible = signal(sessionStorage.getItem('driveos.crm.leads.states.visible') !== 'false');
  readonly advancedFiltersOpen = signal(false);
  readonly exportInProgress = signal(false);
  readonly exportError = signal(false);
  readonly canExport = computed(() => this.authorization.hasPermission(CRM_PERMISSIONS.leads.export));
  readonly canCreate = computed(() => this.authorization.hasPermission(CRM_PERMISSIONS.leads.create)
    && this.selectedState() !== 'readOnly' && this.selectedState() !== 'limitedPermission');
  readonly searchControl = new FormControl('', { nonNullable: true });
  readonly filters = this.formBuilder.nonNullable.group({
    status: ['' as LeadStatus | ''], sourceType: ['' as LeadSourceType | ''],
    branchId: [''], assignedAdvisorId: [''], unassignedOnly: [false],
  });
  readonly selectedLeadIds = signal<readonly string[]>([]);
  readonly statuses: readonly LeadStatus[] = ['New','Contacted','Qualified','AssessmentScheduled',
    'OfferSent','Negotiation','Won','Lost','Dormant','NotEligible','OutOfScope','Duplicate',
    'TransferredToPartner','NoResponse','CancelledByLead','ConvertedElsewhere'];
  readonly sources: readonly LeadSourceType[] = ['Website','DriveOsForm','PhoneCall','WalkIn',
    'Referral','SocialMedia','AdvertisingCampaign','PartnerDrivingSchool','FreelanceInstructor',
    'ExternalImport','Other'];
  readonly viewLoading = computed(() => this.store.isLoading() || this.selectedState() === 'loading');
  ...
  readonly activeFilterCount = computed(() => Object.values(this.filters.getRawValue())
  ...
  constructor() {
    ...
    this.filters.valueChanges.pipe(takeUntilDestroyed(this.destroyRef)).subscribe((value) =>
      this.store.setFilters(value.status ?? '', value.sourceType ?? '', value.unassignedOnly ?? false,
        value.branchId ?? '', value.assignedAdvisorId ?? ''));
  }
  ...
  toggleStateTabs(): void {
    const visible = !this.stateTabsVisible(); this.stateTabsVisible.set(visible);
    sessionStorage.setItem('driveos.crm.leads.states.visible', `${visible}`);
  }
  toggleAdvancedFilters(): void { this.advancedFiltersOpen.update((value) => !value); }
  clearFilters(): void { this.filters.reset(); this.searchControl.setValue(''); }
  exportCsv(): void {
    if (!this.canExport() || this.exportInProgress()) return;
    this.exportInProgress.set(true); this.exportError.set(false);
    this.leadsApi.exportCsv(this.store.parameters()).subscribe({
      next: (blob) => {
        const url = URL.createObjectURL(blob); const anchor = document.createElement('a');
        anchor.href = url; anchor.download = `driveos-prospects-${new Date().toISOString().slice(0, 10)}.csv`;
        anchor.click(); URL.revokeObjectURL(url); this.exportInProgress.set(false);
      },
      error: () => { this.exportInProgress.set(false); this.exportError.set(true); },
    });
  }
  toggleLeadSelection(id: string, checked: boolean): void {
    this.selectedLeadIds.update((ids) => checked ? [...new Set([...ids, id])] : ids.filter((x) => x !== id));
  }
  toggleAll(checked: boolean): void {
    this.selectedLeadIds.set(checked ? this.displayedLeads().map((lead) => lead.id) : []);
  }
  isSelected(id: string): boolean { return this.selectedLeadIds().includes(id); }
  ...
  statusVariant(status: LeadStatus): DriveOsBadgeVariant {
    ...
    if (['Lost','NotEligible','OutOfScope','CancelledByLead','ConvertedElsewhere'].includes(status)) return 'danger';
