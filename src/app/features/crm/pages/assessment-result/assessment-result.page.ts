...
const EMPTY_RESULT: AssessmentResultContent = {
  ...
  supportNeeds: [],
  theoryHours: null,
  practicalHoursMin: null,
  practicalHoursMax: null,
  simulatorHours: null,
  roadHours: null,
  intermediateAssessments: null,
...
export class AssessmentResultPage {
  ...
  readonly canRead = computed(() =>
    this.auth.hasPermission(CRM_PERMISSIONS.assessments.result.read),
  );
  readonly canCreate = computed(() =>
    this.auth.hasPermission(CRM_PERMISSIONS.assessments.result.create),
  );
  readonly canValidate = computed(() =>
    this.auth.hasPermission(CRM_PERMISSIONS.assessments.result.validate),
  );
  readonly canShare = computed(() =>
    this.auth.hasPermission(CRM_PERMISSIONS.assessments.result.share),
  );
  readonly canCreateOffer = computed(() => this.auth.hasPermission(CRM_PERMISSIONS.offers.create));
  readonly editable = computed(
    () =>
      this.canCreate() &&
      ['None', 'Draft', 'CorrectionRequested'].includes(this.result()?.status ?? 'None'),
  );
  readonly validated = computed(() =>
    ['Validated', 'Shared'].includes(this.result()?.status ?? ''),
  );
  ...
  readonly form = this.fb.nonNullable.group({
    ...
    alternatives: [''],
    prospectComment: ['', Validators.maxLength(8000)],
  ...
  back(): void {
    void this.router.navigate(['/crm/assessments', this.appointmentId, 'perform']);
  }
  ...
  save(): void {
    ...
    this.execute(
      () =>
        this.api.saveResult(this.appointmentId!, {
  ...
  requestCorrection(): void {
    ...
    this.execute(
      () => this.api.requestResultCorrection(this.appointmentId!, current.revision, reason),
  ...
  validate(): void {
    const current = this.result();
    if (!current || !this.appointmentId || !this.canValidate() || current.status !== 'Draft')
      return;
    this.execute(
      () => this.api.validateResult(this.appointmentId!, current.revision),
  ...
  share(): void {
    const current = this.result();
    if (!current || !this.appointmentId || !this.canShare() || current.status !== 'Validated')
      return;
    this.execute(
      () => this.api.shareResult(this.appointmentId!, current.revision),
  ...
  private load(): void {
    if (!this.appointmentId || !this.canRead()) {
  ...
  private execute(
    action: () => ReturnType<AssessmentSessionsApiService['shareResult']>,
    successKey: string,
  ): void {
  ...
  private parseContent(value: string | null): AssessmentResultContent {
    ...
    return parsed && typeof parsed === 'object'
      ? { ...EMPTY_RESULT, ...(parsed as AssessmentResultContent) }
      : { ...EMPTY_RESULT };
  }
