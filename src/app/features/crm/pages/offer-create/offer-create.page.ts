...
import {
  CommercialOffer,
  CommercialOfferLineDraft,
  OfferLineType,
  OfferPriceSource,
} from '../../models/commercial-offer.model';

@Component({
  selector: 'driveos-offer-create-page',
  standalone: true,
  imports: [
    CommonModule,
    ReactiveFormsModule,
...
export class OfferCreatePage {
  ...
  readonly lineTypes: OfferLineType[] = [
    'RegistrationFee',
    'TheoryTraining',
    'PracticalLesson',
    'SimulatorLesson',
  ...
  readonly priceSources: OfferPriceSource[] = [
    'StandardCatalog',
    'BranchCatalog',
    'NegotiatedPrice',
    'Campaign',
  ...
  readonly form = this.fb.nonNullable.group({
    ...
    estimatedFundingAmount: [0, Validators.min(0)],
    financingNotes: ['', Validators.maxLength(4000)],
    conditions: ['', Validators.maxLength(8000)],
    internalNotes: ['', Validators.maxLength(8000)],
    lines: this.fb.array<any>([]),
  });
  ...
  constructor() {
    if (!this.leadId || !this.appointmentId) {
  ...
  addLine(type: OfferLineType = 'PracticalLesson'): void {
    this.lines.push(
      this.fb.nonNullable.group({
        ...
        mandatory: [false],
        priceSource: ['StandardCatalog' as OfferPriceSource, Validators.required],
  ...
  removeLine(index: number): void {
    if (!this.lines.at(index).value.mandatory) this.lines.removeAt(index);
  }
  previous(): void {
  ...
  back(): void {
    void this.router.navigate(['/crm/assessments', this.appointmentId, 'result']);
  }

  catalogAmount(): number {
    return this.lineValues().reduce((sum, line) => sum + line.quantity * line.unitPrice, 0);
  }
  discountAmount(): number {
    return this.lineValues().reduce((sum, line) => sum + line.discountAmount, 0);
  }
  taxAmount(): number {
    return this.lineValues().reduce((sum, line) => {
      const net = Math.max(0, line.quantity * line.unitPrice - line.discountAmount);
      return sum + (net * line.taxRate) / 100;
    }, 0);
  }
  totalAmount(): number {
    return Math.max(0, this.catalogAmount() - this.discountAmount()) + this.taxAmount();
  }
  remainingAmount(): number {
    return Math.max(
      0,
      this.totalAmount() - Number(this.form.controls.estimatedFundingAmount.value),
    );
  }
  ...
  generate(): void {
    ...
    this.offers
      .generate(this.leadId, {
        assessmentSessionId: result.sessionId,
        branchId: value.branchId.trim() || null,
        trainingCode: value.trainingCode.trim(),
        currency: value.currency.trim().toUpperCase(),
        validUntilUtc: new Date(`${value.validUntil}T23:59:59`).toISOString(),
        estimatedFundingAmount: Number(value.estimatedFundingAmount),
        financingNotes: value.financingNotes.trim() || null,
      ...
      .subscribe({
        next: (offer) => {
          this.preview.set(offer);
          this.toast.success(this.translate.instant('crm.offers.messages.created'));
        },
  ...
  private seedFromAssessment(result: AssessmentResult): void {
    ...
    this.form.patchValue({
      trainingCode: String(content?.['recommendedTraining'] ?? ''),
  ...
  private manualReasonsValid(): boolean {
    return this.lineValues().every(
      (line) => line.priceSource !== 'ManualOverride' || !!line.manualOverrideReason,
    );
  }
