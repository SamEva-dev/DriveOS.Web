...
export class LeadDetailPage {
  ...
  readonly canReadActivities = computed(() =>
    this.authorization.hasPermission(CRM_PERMISSIONS.activities.read),
  );
  ...
  readonly activityForm = this.formBuilder.nonNullable.group({
    ...
    direction: this.formBuilder.nonNullable.control<CrmActivityDirection>(
      'Outbound',
      Validators.required,
    ),
  ...
  readonly canReadTasks = computed(() =>
    this.authorization.hasPermission(CRM_PERMISSIONS.tasks.read),
  );
  readonly canCreateTask = computed(() =>
    this.authorization.hasPermission(CRM_PERMISSIONS.tasks.create),
  );
  readonly canCompleteTask = computed(() =>
    this.authorization.hasPermission(CRM_PERMISSIONS.tasks.complete),
  );
  readonly canCancelTask = computed(() =>
    this.authorization.hasPermission(CRM_PERMISSIONS.tasks.cancel),
  );
  ...
  readonly canQualify = computed(() =>
    this.authorization.hasPermission(CRM_PERMISSIONS.leads.qualify),
  );
  readonly canConvert = computed(() =>
    this.authorization.hasPermission(CRM_PERMISSIONS.conversions.convertToStudent),
  );
  readonly qualificationForm = this.formBuilder.nonNullable.group({
    ...
    targetDate: [''],
    financing: this.formBuilder.nonNullable.control<FinancingOption>(
      'Unknown',
      Validators.required,
    ),
    notes: ['', Validators.maxLength(2000)],
  ...
  openTaskForm(): void {
    this.taskForm.reset({ type: 'Call', title: '', notes: '', dueAtUtc: '' });
  ...
  saveTask(): void {
    if (!this.leadId || this.taskForm.invalid) {
    ...
    this.tasksApi
      .create(this.leadId, {
        ...
        dueAtUtc: new Date(value.dueAtUtc).toISOString(),
      ...
      .subscribe({
        next: () => {
          ...
          this.isCreatingTask.set(false);
          this.loadTasks();
          this.toast.success(this.translate.instant('crm.tasks.created'));
        },
  ...
  isOverdue(task: CrmTask): boolean {
    return task.status === 'Pending' && new Date(task.dueAtUtc).getTime() < Date.now();
  }
  ...
  saveActivity(): void {
    if (!this.leadId || this.activityForm.invalid) {
    ...
    this.activitiesApi
      ...
      .subscribe({
        next: () => {
          ...
          this.isCreatingActivity.set(false);
          this.loadActivities();
          this.toast.success(this.translate.instant('crm.activities.created'));
        },
        error: (error: HttpErrorResponse) => {
          this.isSavingActivity.set(false);
          this.showErrors(error);
