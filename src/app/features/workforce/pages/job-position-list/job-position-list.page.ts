...
export class JobPositionListPage {
  ...
  readonly canManage = computed(() =>
    this.authorization.hasPermission(WORKFORCE_PERMISSIONS.jobPositions.manage),
  );
  readonly functions = [
    'Instructor',
    ...
    return this.positions().filter(
      (x) =>
        ...
        (!q ||
          ...
          x.professionalFunction.toLowerCase().includes(q)),
    );
  });
  ...
  openCreate(): void {
    ...
    this.form.reset({ code: '', name: '', description: '', professionalFunction: 'Instructor' });
    this.open('create');
  }
  openEdit(item: JobPosition): void {
  ...
  openState(item: JobPosition): void {
    this.selected.set(item);
    this.open(item.status === 'Active' ? 'deactivate' : 'reactivate');
  }
  close(): void {
  ...
  save(): void {
    ...
    const req = {
      ...
      professionalFunction: v.professionalFunction,
    };
    this.run(
      this.drawer() === 'create'
        ? this.api.createJobPosition(req)
        : this.api.updateJobPosition(this.selected()!.id, req),
    );
  }
  confirmState(): void {
    ...
    this.run(
      this.drawer() === 'deactivate'
        ? this.api.deactivateJobPosition(item.id)
        : this.api.reactivateJobPosition(item.id),
    );
  }
  tone(status: string): DriveOsStatusTone {
    return status === 'Active' ? 'success' : 'neutral';
  }
  drawerTitleKey(): string {
    return `workforce.jobPositions.drawers.${this.drawer() ?? 'create'}.title`;
  }
  private open(kind: Exclude<DrawerKind, null>): void {
    this.actionErrors.set([]);
