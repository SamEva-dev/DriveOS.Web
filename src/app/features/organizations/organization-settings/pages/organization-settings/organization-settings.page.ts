...
export class OrganizationSettingsPage {
  ...
  readonly profileForm = new FormGroup({
    ...
    registrationNumber: new FormControl('', { nonNullable: true, validators: [Validators.maxLength(100)] }),
  ...
  readonly addressForm = new FormGroup({
    ...
    addressCountryCode: new FormControl('FR', { nonNullable: true, validators: [Validators.required, Validators.pattern(/^[A-Za-z]{2}$/)] }),
  });

  readonly regionalForm = new FormGroup({
    defaultLanguage: new FormControl('fr', { nonNullable: true, validators: [Validators.required] }),
    supportedLanguagesText: new FormControl('fr', { nonNullable: true, validators: [Validators.required] }),
    timeZoneId: new FormControl('Europe/Paris', { nonNullable: true, validators: [Validators.required] }),
    currencyCode: new FormControl('EUR', { nonNullable: true, validators: [Validators.required, Validators.pattern(/^[A-Za-z]{3}$/)] }),
    ...
    firstDayOfWeek: new FormControl(1, { nonNullable: true, validators: [Validators.min(0), Validators.max(6)] }),
    measurementSystem: new FormControl<MeasurementSystem>(0, { nonNullable: true }),
  });

  readonly operationalForm = new FormGroup({
    defaultSessionDurationMinutes: new FormControl(60, { nonNullable: true, validators: [Validators.required, Validators.min(15), Validators.max(480)] }),
    defaultBookingLeadTimeMinutes: new FormControl(60, { nonNullable: true, validators: [Validators.required, Validators.min(0)] }),
    defaultCancellationDelayHours: new FormControl(24, { nonNullable: true, validators: [Validators.required, Validators.min(0)] }),
  ...
  initialize(): void {
    ...
    const defaultBranch = this.branches().find(branch => branch.isPrimary) ?? this.branches()[0] ?? null;
    this.initializing.set(true);

    this.settingsApi.create(this.organizationId, {
      tradeName: null,
      ...
      .pipe(finalize(() => this.initializing.set(false)), takeUntilDestroyed(this.destroyRef))
      .subscribe({
        next: () => {
          this.toast.success(this.translate.instant('organizations.settings.notifications.created'));
          this.loadSettings();
        },
  ...
  saveProfile(): void {
    ...
    this.settingsApi.updateProfile(this.organizationId, {
    ...
    }).pipe(finalize(() => this.savingSection.set(null)), takeUntilDestroyed(this.destroyRef))
  ...
  saveContact(): void {
    ...
    this.settingsApi.updateContact(this.organizationId, {
      email: this.nullIfBlank(value.email), phone: this.nullIfBlank(value.phone),
      website: this.nullIfBlank(value.website), expectedVersion: this.currentVersion(),
    }).pipe(finalize(() => this.savingSection.set(null)), takeUntilDestroyed(this.destroyRef))
      .subscribe({ next: () => this.afterSave(), error: error => this.showErrors(error) });
  }
  ...
  saveAddress(): void {
    ...
    this.settingsApi.updateAddress(this.organizationId, {
      addressLine1: this.nullIfBlank(value.addressLine1), addressLine2: this.nullIfBlank(value.addressLine2),
      postalCode: this.nullIfBlank(value.postalCode), city: this.nullIfBlank(value.city),
      region: this.nullIfBlank(value.region), addressCountryCode: value.addressCountryCode.trim().toUpperCase(),
      expectedVersion: this.currentVersion(),
    }).pipe(finalize(() => this.savingSection.set(null)), takeUntilDestroyed(this.destroyRef))
      .subscribe({ next: () => this.afterSave(), error: error => this.showErrors(error) });
  }
  ...
  saveRegional(): void {
    ...
    if (!supportedLanguages.includes(value.defaultLanguage.trim().toLowerCase())) {
      this.toast.error(this.translate.instant('errors.title'), this.translate.instant('organizations.settings.errors.defaultLanguageNotSupported'));
      this.savingSection.set(null);
      return;
    }
    this.settingsApi.updateRegional(this.organizationId, {
      defaultLanguage: value.defaultLanguage.trim().toLowerCase(), supportedLanguages,
      timeZoneId: value.timeZoneId.trim(), currencyCode: value.currencyCode.trim().toUpperCase(),
      dateFormat: value.dateFormat.trim(), timeFormat: value.timeFormat.trim(),
      firstDayOfWeek: Number(value.firstDayOfWeek), measurementSystem: Number(value.measurementSystem) as MeasurementSystem,
      expectedVersion: this.currentVersion(),
    }).pipe(finalize(() => this.savingSection.set(null)), takeUntilDestroyed(this.destroyRef))
      .subscribe({ next: () => this.afterSave(), error: error => this.showErrors(error) });
  }
  ...
  saveOperational(): void {
    ...
    if (value.requireBranchForOperations && !value.defaultBranchId) {
      this.toast.error(this.translate.instant('errors.title'), this.translate.instant('organizations.settings.errors.defaultBranchRequired'));
      this.savingSection.set(null);
      return;
    }
    this.settingsApi.updateOperational(this.organizationId, {
  ...
  private loadSettings(): void {
    this.settingsApi.get(this.organizationId)
  ...
  private patchForms(settings: OrganizationSettings): void {
    this.profileForm.reset({ tradeName: settings.tradeName ?? '', registrationNumber: settings.registrationNumber ?? '', taxNumber: settings.taxNumber ?? '' });
    this.contactForm.reset({ email: settings.email ?? '', phone: settings.phone ?? '', website: settings.website ?? '' });
    this.addressForm.reset({ addressLine1: settings.addressLine1 ?? '', addressLine2: settings.addressLine2 ?? '', postalCode: settings.postalCode ?? '', city: settings.city ?? '', region: settings.region ?? '', addressCountryCode: settings.addressCountryCode });
    this.regionalForm.reset({ defaultLanguage: settings.defaultLanguage, supportedLanguagesText: settings.supportedLanguages.join(', '), timeZoneId: settings.timeZoneId, currencyCode: settings.currencyCode, dateFormat: settings.dateFormat, timeFormat: settings.timeFormat, firstDayOfWeek: settings.firstDayOfWeek, measurementSystem: settings.measurementSystem });
    this.operationalForm.reset({ defaultSessionDurationMinutes: settings.defaultSessionDurationMinutes, defaultBookingLeadTimeMinutes: settings.defaultBookingLeadTimeMinutes, defaultCancellationDelayHours: settings.defaultCancellationDelayHours, allowStudentSelfBooking: settings.allowStudentSelfBooking, requireBranchForOperations: settings.requireBranchForOperations, defaultBranchId: settings.defaultBranchId });
  ...
  private currentVersion(): number { return this.settings()?.version ?? 0; }
  private nullIfBlank(value: string): string | null { const normalized = value.trim(); return normalized ? normalized : null; }
  private parseLanguages(value: string): string[] { return [...new Set(value.split(',').map(x => x.trim().toLowerCase()).filter(Boolean))]; }
  private markAllPristine(): void { [this.profileForm, this.contactForm, this.addressForm, this.regionalForm, this.operationalForm].forEach(form => form.markAsPristine()); }
  private showErrors(error: HttpErrorResponse): void { for (const message of this.apiErrorService.getMessages(error)) this.toast.error(this.translate.instant('errors.title'), message); }
}
