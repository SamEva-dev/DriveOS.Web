...
import { BRANCH_TYPE_OPTIONS, BranchType } from '../../models/branch-type';

export type BranchFormMode = 'create' | 'edit';

@Component({
  selector: 'driveos-branch-form',
...
export class BranchFormComponent {
  ...
  readonly mode = input<BranchFormMode>('create');

  readonly initialValue = input<BranchFormValue | null>(null);

  readonly submitting = input(false);

  readonly submitted = output<BranchFormValue>();

  readonly cancelled = output<void>();

  readonly isCreateMode = computed(() => this.mode() === 'create');

  readonly branchTypeOptions = computed(() =>
  ...
  readonly form = this.formBuilder.nonNullable.group({
    ...
    timeZoneId: ['Europe/Paris', [Validators.required, Validators.maxLength(100)]],
  ...
  constructor() {
    effect(() => {
      ...
      this.form.reset({
        ...
        branchType: initialValue.branchType,
        ...
        isPrimary: initialValue.isPrimary,
      });
    });
  }
  ...
  submit(): void {
    ...
    this.submitted.emit({
      ...
      branchType: rawValue.branchType,
