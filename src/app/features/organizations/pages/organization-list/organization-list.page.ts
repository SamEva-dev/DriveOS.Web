...
@Component({
  ...
  providers: [OrganizationsListStore],
...
export class OrganizationListPage {
  readonly store = inject(OrganizationsListStore);
  ...
  toggleSorting(field: OrganizationSortField): void {
  ...
  sortingIcon(field: OrganizationSortField): string {
  ...
  organizationTypeKey(type: string): string {
    ...
    return keys[type] ?? 'organizations.types.unknown';
  }

  organizationStatusKey(status: string): string {
