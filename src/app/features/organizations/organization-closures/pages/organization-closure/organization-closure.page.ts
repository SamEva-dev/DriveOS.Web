...
@Component({ selector: 'drive-os-organization-closure', standalone: true, templateUrl: './organization-closure.page.html', changeDetection: ChangeDetectionStrategy.OnPush })
export class OrganizationClosurePage {
  ...
  load(): void { this.loading.set(true); this.api.list(this.organizationId).subscribe({ next: x => this.closures.set(x), complete: () => this.loading.set(false), error: () => this.loading.set(false) }); }
}
