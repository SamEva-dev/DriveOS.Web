...
import { BranchAssignmentSummaryComponent } from '../../components/branch-assignment-summary/branch-assignment-summary.component';

import { BranchAssignmentTimelineComponent } from '../../components/branch-assignment-timeline/branch-assignment-timeline.component';

import { BranchAssignmentUserCardComponent } from '../../components/branch-assignment-user-card/branch-assignment-user-card.component';
...
import { buildBranchAssignmentTimeline } from '../../domain/branch-assignment-timeline';
...
@Component({
  selector: 'app-branch-assignment-detail-page',
  ...
  templateUrl: './branch-assignment-detail.page.html',

  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class BranchAssignmentDetailPage implements OnInit {
  private readonly route = inject(ActivatedRoute);

  private readonly api = inject(BranchUserAssignmentsApiService);

  private readonly authUsersApi = inject(AuthUsersApiService);
  ...
  readonly assignment = signal<BranchUserAssignment | null>(null);
  ...
  readonly assignmentErrors = signal<readonly string[]>([]);

  readonly userErrors = signal<readonly string[]>([]);

  readonly timeline = computed<readonly BranchAssignmentTimelineItem[]>(() => {
    const assignment = this.assignment();

    return assignment ? buildBranchAssignmentTimeline(assignment) : [];
  });
  ...
  readonly teamLink = computed(() => {
    ...
    return assignment
      ? ['/organizations', this.organizationId, 'branches', assignment.branchId, 'team']
  ...
  readonly branchLink = computed(() => {
    ...
    return assignment
      ? ['/organizations', this.organizationId, 'branches', assignment.branchId]
      : ['/organizations', this.organizationId];
  });
  ...
  openAction(action: BranchAssignmentLifecycleAction): void {
    const assignment = this.assignment();
  ...
  executeAction(event: {
    ...
    const operation =
      event.action === 'suspend'
        ? this.api.suspend(this.organizationId, event.assignmentId, request)
        : event.action === 'reactivate'
          ? this.api.reactivate(this.organizationId, event.assignmentId, request)
          : this.api.end(this.organizationId, event.assignmentId, request);
  ...
  private load(): void {
    ...
    this.api.getById(this.organizationId, this.assignmentId).subscribe({
      next: (assignment) => {
        this.assignment.set(assignment);

        this.loadUser(assignment.userId);
      },
      ...
      error: (error) => {
        ...
        this.assignmentErrors.set(this.apiError.getMessages(error));
      },
    });
  }
    ...
    this.authUsersApi
      ...
      .subscribe({
        ...
        error: (error) => {
          ...
          this.userErrors.set(this.apiError.getMessages(error));
        },
      });
  }
