...
import {
  DriveOsToastService,
} from './driveos-toast.service';

@Component({
  selector: 'dos-toast-container',
  standalone: true,
...
export class DriveOsToastContainerComponent {
  ...
  containerClasses(
    variant: DriveOsToastVariant,
  ): string {
    const classes:
      Record<DriveOsToastVariant, string> = {
      success:
  ...
  iconClass(
    ...
    const classes:
      ...
      info:
        'ph-fill ph-info text-xl',

      warning:
        'ph-fill ph-warning text-xl',

      error:
        'ph-fill ph-x-circle text-xl',
    };
